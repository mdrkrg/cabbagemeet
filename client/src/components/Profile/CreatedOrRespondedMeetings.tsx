...
export default function CreatedMeetings({ showCreatedMeetings }: { showCreatedMeetings: boolean }) {
  const createdMeetingsReqInfo = useGetCreatedMeetingsQuery(undefined, {
    skip: !showCreatedMeetings,
  });
  const respondedMeetingsReqInfo = useGetRespondedMeetingsQuery(undefined, {
    skip: showCreatedMeetings,
  });
  const { data, isError, error } = showCreatedMeetings
    ? createdMeetingsReqInfo
    : respondedMeetingsReqInfo;

  if (isError) {
    // TODO: add a Retry button
    return <p className="my-auto">An error occurred: {getReqErrorMessage(error!)}</p>;
  } else if (!data) {
      ...
      <div className="my-auto">
        ...
        {showCreatedMeetings && (
          <Link to="/" className="d-block mt-4 text-decoration-none">
            <button className="btn btn-primary d-block mx-auto">Create a meeting</button>
          </Link>
        )}
      </div>
    );
          ...
          <div className={`ps-4 ${styles.meetingCardRight}`}>
            ...
            {meeting.scheduledStartDateTime === undefined && (
              <div className="d-flex align-items-start">
                ...
                <div className="ms-2">{meetingDatesRangeString(meeting.tentativeDates)}</div>
              </div>
            )}
            <div className="d-flex align-items-start">
              <div>
              ...
              <div className="ms-2">{meetingTimesRangeString(meeting)}</div>
            </div>
            <div>
              <GlobeIcon />
              <span className="ms-2">{tzAbbr}</span>
            </div>
          </div>
        </Link>
      ))}
...
function ScheduleInfo({ meeting }: { meeting: TransformedMeetingShortResponse }) {
  if (!meeting.scheduledStartDateTime) {
    return <p className={"my-auto pe-3 pe-md-4 " + styles.notScheduledText}>Not scheduled</p>;
  }
