...
function cellIsInSelectionArea(rowIdx: number, colIdx: number, state: MouseState): boolean {
  return (
    (state.type === "down" || state.type === "upCellsSelected") &&
...
function cellIsInSelectionColumn(rowIdx: number, colIdx: number, state: MouseState): boolean {
  return (
    (state.type === "down" || state.type === "upCellsSelected") &&
    ((state.downCell.rowIdx <= rowIdx && rowIdx <= state.curCell.rowIdx) ||
      (state.curCell.rowIdx <= rowIdx && rowIdx <= state.downCell.rowIdx)) &&
...
function calculateDateTimeGrid(
  numRows: number,
  numCols: number,
  dateStrings: string[],
  startHour: number,
  ...
  for (let rowIdx = 0; rowIdx < numRows; rowIdx++) {
    ...
    for (let colIdx = 0; colIdx < numCols; colIdx++) {
      ...
      const dateTime = customToISOString(date, hour, rowIdx % 2 === 0 ? 0 : 30);
      row.push(dateTime);
    }
    rows.push(row);
  }
...
function MeetingGridBodyCells({
  ...
  const { respondents, minStartHour, maxEndHour, scheduledStartDateTime, scheduledEndDateTime } =
    ...
    <>
      {gridCoords.map(([colIdx, rowIdx], i) => {
        ...
        const scheduledStartDateTimeProp =
          roundedDownScheduledStartDateTime === dateTime ? scheduledStartDateTime : undefined;
        const externalEvents =
          dateTimesToExternalEventInfo[dateTime] || emptyArrayOfExternalEventInfo;
        const hoverUserIsAvailableAtThisTime =
          somebodyIsHovered && respondents[hoverUser].availabilities[dateTime];
...
const Cell = React.memo(function Cell({
  ...
  const selMode = useAppSelector(selectSelMode);
  const { selfRespondentID } = useGetCurrentMeetingWithSelector(({ data }) => ({
    selfRespondentID: data?.selfRespondentID,
  }));
  ...
  if (showRespondentsColour) {
    ...
    const alpha =
      Math.round(100 * (0.2 + 0.8 * (numPeopleAvailableAtThisTime / totalPeople))) + "%";
  ...
  if (
    ...
    externalEventBoxes = externalEvents.events.map((externalEvent, i) => {
      const { topOffset, height } = calculateTopOffsetAndHeightOfExternalEventBox(
        dateTime,
        externalEvent.startDateTime,
        externalEvent.endDateTime,
      );
      return (
        <div
          key={i}
