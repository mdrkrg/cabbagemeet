...
describe("OAuth2Controller (e2e) (Microsoft)", () => {
  ...
  it("/api/login-with-microsoft (POST) (user exists, not linked yet, refresh token is present)", async () => {
    ...
    expect(
      redirect2.startsWith("http://cabbagemeet.internal/confirm-link-microsoft-account?"),
    ).toBe(true);
    const redirect2Params = decodeQueryParams(
      redirect2.slice("http://cabbagemeet.internal/confirm-link-microsoft-account?".length),
    );
        ...
        value: [
          {
            ...
            start: {
              dateTime:
                schedule.startDateTime.slice(0, schedule.startDateTime.length - 1) + ".000000",
              timeZone: "UTC",
            },
            end: {
