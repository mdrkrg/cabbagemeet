...
export async function editUser(
  ...
  const { body } = await PATCH("/api/me", app, token).send(editUserDto).expect(HttpStatus.OK);
  return body;
}
...
export async function getMeeting(
  ...
  const { body } = await GET("/api/meetings/" + meetingID, app, token).expect(HttpStatus.OK);
  return body;
}
