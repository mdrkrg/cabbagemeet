...
function stateIsEditing(state: AvailabilitiesSelectionState): state is {
  ...
  return (
    state.selMode.type === "editingRespondent" ||
...
export const availabilitiesSelectionSlice = createSlice({
  ...
  reducers: {
    ...
    editUserInternal: (
      state,
      {
        payload: { availabilities, isAdding, selfRespondentID },
      }: PayloadAction<{
      ...
      let selMode:
        | { type: "addingRespondent" }
        | { type: "editingRespondent"; respondentID: number }
        | undefined;
      if (isAdding) {
        assert(state.selMode.type === "none");
    ...
    notifyMouseDown: (
      state,
      { payload }: PayloadAction<{ cell: CellCoord; wasOriginallySelected: boolean }>,
    ) => {
      assert(stateIsEditing(state));
      state.mouse = {
        type: "down",
...
const { editUserInternal } = availabilitiesSelectionSlice.actions;
...
export function useEditSelectedUser() {
  ...
  const { availabilities } = useGetCurrentMeetingWithSelector(({ data: meeting }) => ({
    availabilities:
...
export function useEditSelfWhenLoggedIn() {
  ...
  const { availabilities, selfRespondentID } = useGetCurrentMeetingWithSelector(
    ({ data: meeting }) => ({
      availabilities:
        meeting?.selfRespondentID !== undefined
