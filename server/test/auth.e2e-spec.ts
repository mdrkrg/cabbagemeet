...
http://cabbagemeet.internal/verify-email?`,
    ...
    await POST("/api/verify-email", app).send(verifyBody).expect(HttpStatus.NO_CONTENT);
  });
  ...
  it("/api/logout (POST)", async () => {
    ...
    await POST("/api/logout?everywhere=true", app, token).expect(HttpStatus.NO_CONTENT);
  ...
  it("/api/reset-password (POST)", async () => {
    await POST("/api/reset-password", app).send({ email: "b@c" }).expect(HttpStatus.NO_CONTENT);
    ...
    await POST("/api/reset-password", app).send({ email: "a@b" }).expect(HttpStatus.NO_CONTENT);
...
http://cabbagemeet.internal/confirm-password-reset?pwresetToken=`,
    ...
    await POST("/api/login", app).send({ email: "a@b", password: "bcdefg" }).expect(HttpStatus.OK);
  });
