...
export class Oauth2Controller {
  ...
  private async handleRedirectForLogin(
    ...
    if (loginResult.type === OIDCLoginResultType.USER_EXISTS_AND_IS_LINKED) {
      ...
      res.redirect(await this.oauth2Service.getRequestURL(providerType, state, true));
      ...
      res.redirect(
        `${this.publicURL}/confirm-link-${providerName}-account?` + encodeQueryParams(urlParams),
      );
    } else if (
      loginResult.type === OIDCLoginResultType.USER_DOES_NOT_EXIST_AND_NEED_A_NEW_REFRESH_TOKEN
    ) {
  ...
  private async handleRedirectFromOAuth2Provider(
    ...
    try {
      ...
      if (err instanceof OAuth2NotConfiguredError) {
        res.redirect(`${this.publicURL}/error?e=E_OAUTH2_NOT_AVAILABLE&provider=${providerParam}`);
